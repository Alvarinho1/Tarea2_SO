...
// Función para procesar una parte específica de la imagen
void process_image_part(const cv::Mat& input_image, cv::Mat& output_image,
...
void process_image(const std::string& input_image, const std::string& output_image, int num_threads) {
    ...
    for (int i = 0; i < num_threads; ++i) {
        ...
        auto thread_function = [&image, &gray_image, start_row, end_row]() {
            // Procesar la parte asignada de la imagen
            process_image_part(image, gray_image, start_row, end_row);
        };
